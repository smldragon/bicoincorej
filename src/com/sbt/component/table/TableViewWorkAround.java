...
public abstract class TableViewWorkAround {
    ...
    public static void patchContextMenuLeak(TableView<?> table) {
        ...
        if ( null != tableHeaderRow) {
            ...
            for(int i=0;i<table.getColumns().size();i++) {
                TableColumn<?,?> tc = table.getColumns().get(i);
                setDragEventHandler(table,tableRowHeaderContextMenu,tc);
            }
        }
    }
    private static void setDragEventHandler(TableView<?> table, PopupControl popupControl,TableColumn<?,?> tableColumn) {

        TableColumnHeader tch = TableUtil.getColumnHeader(table,tableColumn);
        if ( null !=  tch ) {
            ...
            if ( null != subColumns) {
                for(int i=0;i<subColumns.size();i++) {
                    setDragEventHandler(table,popupControl,(TableColumn<?,?>)subColumns.get(i));
                }
            }
        }
    }
